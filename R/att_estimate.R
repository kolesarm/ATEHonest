...
#' @param h The output of \code{ATTh} on which to base the estimate
#' @template data
#' @export
ATTOptPath <- function(h, y, d) {
    n <- length(d)
    n0 <- n-sum(d)

    rmean <- rowMeans(h$res[, (n0+2):(n+1), drop=FALSE])
    m <- h$res[, 2:(n0+1), drop=FALSE]
    maxbias <- rmean - apply(m, 1, function(x) sum(x^2)) / rowSums(m)
    resw <- ATTOptW(h$res, d)

    list(ep=data.frame(att=drop(resw %*% y), maxbias=maxbias,
                       delta=unname(h$res[, 1]),
                       omega=unname(2*(h$res[, n+2]+rmean))),
         resw=resw, y=y, d=d, res=h$res)
}
...
#' ## Use NSW experimental subsample with 20 treated and untreated units
...
#' h <- ATTh(D0, maxiter=120)
#' op <- ATTOptPath(h, dt$re78, dt$treated)
...
ATTOptEstimate <- function(op, sigma2, C=1, sigma2final=sigma2, alpha=0.05,
    ...
    up <- function(res) {
        r <- ATTOptPath(list(res=res), op$y, op$d)
...
#' (2018)
#' @inheritParams ATTMatchEstimate
#' @param h The output of \code{ATTh}.
...
ATTEffBounds <- function(h, d, sigma2, C=1, beta=0.8, alpha=0.05) {
    ...
    del0 <- h$res[, "delta"]
    mu0 <- h$res[, "mu"]
    m0 <- h$res[, 2:(n0+1), drop=FALSE]
    r0 <- h$res[, (n0+2):(n+1), drop=FALSE]
